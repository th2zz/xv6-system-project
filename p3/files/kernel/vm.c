...
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  for(; a  < oldsz; a += PGSIZE){
    ...
    if(pte && (*pte & PTE_P) != 0){
      ...
      if(pa == 0)
        panic("kfree");
      //avoid freeing shared pages
      int is_shared = 0;
      for(int i = 0; i < 3; i++){
      	if(proc->shm_physical[i] == pa && proc->shm_mapped[i] == 1){
		is_shared = 1;
		break;
	}
      }
      if(is_shared == 1) continue;//avoid freeing shared pages
      kfree((char*)pa);
      *pte = 0;
    }
  }
...
freevm(pde_t *pgdir)
{
  ...
  deallocuvm(pgdir, USERTOP, 4*PGSIZE);//avoid freeing shared pages
...
void* shmget(int page_number) {
	...
	//entry is allocated 
	if(proc->shm_mapped[page_number] == 1) return (void*)proc->shm_virtual[page_number];	
	else {
		mappages(proc->pgdir, (void*)proc->shm_virtual[page_number], PGSIZE, (uint)proc->shm_physical[page_number], PTE_W|PTE_U);
		proc->shm_mapped[page_number] = 1;
		return (void*)proc->shm_virtual[page_number];
	}
}
